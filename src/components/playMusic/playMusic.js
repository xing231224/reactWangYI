...
(function () {
  window.psAudio = function (obj) {
    ...
    $("body").append(
      $(
        '<a class="audio-head-tittle-text-out"></a><a class="audio-head-tittle-text-out-a"></a>'
      )
    );
    $("#audio").append(
      $(
        '<audio id="audio-my" src="" preload="metadata"></audio><div class="audio-head"></div><div class="add"></div><div class="min-time"></div><div class="audio-img"></div><div class="audio-text"></div><div class="audio-by"></div><div class="audio-btn"></div><div class="audio-sound"></div>'
      )
    );
    $(".audio-head").append(
      $(
        '<div class="audio-head-tittle"><div class="audio-head-tittle-text audio-head-tittle-text-off"></div></div><div class="audio-head-tittle-by"></div><div class="audio-play-list audio-play-list-off"><div class="audio-play-list-all"></div></div>'
      )
    );
    $(".audio-img").append(
      $(
        '<canvas id="audio-img-canvas" width="200px" height="200px"></canvas><div class="audio-img-cover audio-img-cover-off"></div><canvas id="audio-img-canvas-play" class="audio-img-canvas-play-off" width="45px" height="200px"></canvas>'
      )
    );
    $(".audio-by").append(
      $(
        '<div class="audio-by-all"><span class="audio-by-now"></span></div><div class="audio-by-text"><span class="audio-by-text-now">00:00</span><span class="audio-by-text-all">00:00</span></div>'
      )
    );
    $(".audio-btn").append(
      $(
        '<div class="audio-btn-list audio-btn-list-off"></div><div class="audio-btn-before"></div><div class="audio-btn-play audio-btn-play-off audio-btn-play-off-a"></div><div class="audio-btn-sound"></div><div class="audio-btn-next"></div>'
      )
    );
    $(".audio-sound").append(
      $(
    ...
    $(".audio-img-cover").css({
      background:
        "url(" + audioSrcName + audioName[0] + ".jpg)" + " no-repeat center",
    ...
    function audioState() {
      if (!audioLoop && !random) {
        //  随机播放
        $(".audio-btn-list")
          .removeClass("audio-btn-list-off")
          .addClass("audio-btn-list-fun");
        random = !random;
      } else if (!audioLoop && random) {
        //  循环播放
        $(".audio-btn-list")
          .removeClass("audio-btn-list-fun")
          .addClass("audio-btn-list-on");
        audioLoop = !audioLoop;
        random = !random;
      } else if (audioLoop && !random) {
        //  顺序播放
        $(".audio-btn-list")
          .removeClass("audio-btn-list-on")
          .addClass("audio-btn-list-off");
        audioLoop = !audioLoop;
      }
    }
    function addSong() {
          ...
          if (obj.addAuto && hasSong) {
            ...
            if (!audioPlay) {
              ...
              $(".audio-btn-play")
                .removeClass("audio-btn-play-off")
                .addClass("audio-btn-play-on");
              $("#audio-img-canvas-play").toggleClass(
